 case 1 "LIST分区表(1普通索引)：建立列表分区表" 0
        src_sql:
            drop table objects
            /
 ...
 case 7 "LIST分区表(1普通索引)：在分区中增加键值" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) )
            ...
            /
            insert into objects values ( 2, 'VIEW' )
            /
            commit
            /
        ...
        tgt_sql:
            select high_value from user_tab_partitions where table_name='OBJECTS' and partition_name='TYPE_VIEW'
            /
            select * from objects partition ( type_view ) order by object_id
            /
			
 case 8 "LIST分区表(1普通索引)：在分区中删除键值" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) )
            ...
            /
            insert into objects values (3,'JAVA')
            /
            commit
            /
 ...
 case 9 "LIST分区表(1普通索引)：移动分区（移动到相同表空间）：移动" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) )
                   PARTITION BY LIST ( object_type ) (
                       PARTITION type_table VALUES ('TABLE' ) TABLESPACE users,
                       PARTITION type_index VALUES ('INDEX' ) NOLOGGING,
                       PARTITION type_view  VALUES ('VIEW'  ),
                       PARTITION type_other VALUES ('FUNCTION','TRIGGER','PROCEDURE'),
                       PARTITION type_null  VALUES (NULL)
                   )
            /
            ...
            insert into objects values ( 2, 'VIEW' )
            /
			insert into objects values ( 3, 'FUNCTION' )
            /
            commit
            /
            ALTER TABLE objects MOVE PARTITION type_view
            /
            alter index "objects_idx001" rebuild
            /
 ...
 case 10 "LIST分区表(1普通索引)：移动分区（移动到相同表空间）：验证移动后的数据是否正确" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) )
            ...
            alter index "objects_idx001" rebuild
            /
            update objects partition ( type_view ) set object_id=1000000+object_id
            /
            commit
            /
 ...
 case 11 "LIST分区表(1普通索引)：移动分区（移动到其它表空间）：移动" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) )
            ...
            commit
			/
            ALTER TABLE objects MOVE PARTITION type_view TABLESPACE system
            /
            alter index "objects_idx001" rebuild
            /
            commit
 ...
 case 12 "LIST分区表(1普通索引)：更名分区" 0
        src_sql:
            ...
            commit
			/
            alter table objects rename partition type_view to type_view_new
            /

        tgt_sql:
            select count(*) from user_tab_partitions where table_name='OBJECTS' and partition_name='TYPE_VIEW'
            /
            select count(*) from user_tab_partitions where table_name='OBJECTS' and partition_name='TYPE_VIEW_NEW'
            /
			select * from objects order by object_id
            /


 case 13 "LIST分区表(1普通索引)：分裂分区：第一个分裂出来的分区包含数据：建立分区表" 0
        src_sql:
            drop table objects
            /
 ...
 case 14 "LIST分区表(1普通索引)：分裂分区：第一个分裂出来的分区包含数据：分裂分区" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) ) PARTITION BY LIST ( object_type ) (
 ...
 case 15 "LIST分区表(1普通索引)：分裂分区：第二个分裂出来的分区包含数据：建立分区表" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) ) PARTITION BY LIST ( object_type ) (
 ...
 case 16 "LIST分区表(1普通索引)：分裂分区：第二个分裂出来的分区包含数据：分裂分区" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) ) PARTITION BY LIST ( object_type ) (
 ...
 case 17 "LIST分区表(1普通索引)：分裂分区：分裂出来的2个分区均包含数据：建立分区表" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) ) PARTITION BY LIST ( object_type ) (
 ...
 case 18 "LIST分区表(1普通索引)：分裂分区：分裂出来的2个分区均包含数据：分裂分区" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) ) PARTITION BY LIST ( object_type ) (
 ...
 case 19 "LIST分区表(1普通索引)：TRUNCATE 分区" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) ) PARTITION BY LIST ( object_type ) (
 ...
 case 20 "LIST分区表(1普通索引)：TRUNCATE 分区：TRUNCATE后执行UPDATE" 0
        src_sql:
            drop table objects
            /
            create table objects ( object_id NUMBER, object_type VARCHAR2(19) ) PARTITION BY LIST ( object_type ) (
