case 1 "HASH分区表(一LOCAL索引)：建立：指定分区名称，为每个分区指定表空间" 0
        src_sql:
            drop table "dds_dept"
            /
 ...
 case 2 "HASH分区表(一LOCAL索引)：INSERT 记录" 0
        src_sql:
            drop table "dds_dept"
            /
            create table "dds_dept" ( deptno number, deptname varchar2(32), salmount number )
 ...
 case 3 "HASH分区表(一LOCAL索引)：UPDATE 记录" 0
        src_sql:
            drop table "dds_dept"
            /
            create table "dds_dept" ( deptno number, deptname varchar2(32), salmount number )
 ...
 case 4 "HASH分区表(一LOCAL索引)：增加分区：分区未命名" 0
        src_sql:
            drop table "dds_dept"
            /
            create table "dds_dept" ( deptno number, deptname varchar2(32), salmount number )
 ...
 case 5 "HASH分区表(一LOCAL索引)：增加分区：分区有命名" 0
        src_sql:
            drop table "dds_dept"
            /
            create table "dds_dept" ( deptno number, deptname varchar2(32), salmount number )
 ...
 case 6 "HASH分区表(一LOCAL索引)：分区改名" 0
        src_sql:
            drop table "dds_dept"
            /
            create table "dds_dept" ( deptno number, deptname varchar2(32), salmount number )
 ...
 case 7 "HASH分区表(一LOCAL索引)：分区TRUNCATE" 0
        src_sql:
            drop table "dds_dept"
            /
            create table "dds_dept" ( deptno number, deptname varchar2(32), salmount number )
            ...
            create index dds_dept_idx2 on "dds_dept" ( deptno ) local
            /
            insert into "dds_dept" select object_id, object_name, object_id from dba_objects where rownum<1024
            /
 ...
 case 8 "HASH分区表(一LOCAL索引)：分区: 交换分区: 交换第一个分区" 0
        src_sql:
            drop table swap_t
            /
            drop table "dds_dept"
 ...
 case 9 "HASH分区表(一LOCAL索引)：分区: 交换分区: 交换中间的分区" 0
        src_sql:
            drop table swap_t
            /
            drop table "dds_dept"
            ...
            /
            insert into swap_t
                   select deptno, substr(deptname,1,2)||'GUO' AS deptname,salmount+2000 from "dds_dept" partition (p3)
            /
			commit
			/
 ...
 case 10 "HASH分区表(一LOCAL索引)：分区: 移动分区（移动到相同表空间）:第一个分区" 0
        src_sql:
            drop table swap_t
            /            
			drop table "dds_dept"
 ...
 case 11 "HASH分区表(一LOCAL索引)：分区: 移动分区（移动到相同表空间）:中间分区" 0
        src_sql:
            drop table "dds_dept"
            /
            create table "dds_dept" ( deptno number, deptname varchar2(32), salmount number )
        ...
        tgt_sql:
            select tablespace_name from user_tab_partitions where table_name='dds_dept' and partition_name='P2'
			/
			select count(*) from "dds_dept"
            /
            select to_char(sum(deptno)) from "dds_dept"
            ...
            select count(*) from "dds_dept" partition (p2)
            /
            select to_char(sum(salmount)) from "dds_dept" partition (p2)
            /

 case 12 "HASH分区表(一LOCAL索引)：分区: 移动分区（移动到相同表空间）:最后一个分区" 0
        src_sql:
			drop table "dds_dept"
            /
            create table "dds_dept" ( deptno number, deptname varchar2(32), salmount number )
        ...
        tgt_sql:
            select tablespace_name from user_tab_partitions where table_name='dds_dept' and partition_name='P4'
			/
			select count(*) from "dds_dept"
            /
            select to_char(sum(deptno)) from "dds_dept"
 ...
 case 13 "HASH分区表(一LOCAL索引)：分区: 移动分区（移动到其它表空间）" 0
        src_sql:           
            drop table "dds_dept"
            /
            create table "dds_dept" ( deptno number, deptname varchar2(32), salmount number )
 ...
 case 14 "HASH分区表(一LOCAL索引)：删除HASH分区表" 0
        src_sql:
            drop table "dds_dept"
            /
            create table "dds_dept" ( deptno number, deptname varchar2(32), salmount number )
