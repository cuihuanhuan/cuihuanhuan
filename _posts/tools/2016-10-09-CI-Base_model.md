...
https://github.com/jamierumbelow/codeigniter-base-model(github)

基于Codeigniter的查询辅助函数和查询构造器类的wrapper class

![img](/images/img/ci-db-util.png)

那上代码吧,代码比语言更清晰

My_Model.php

将My_Model.php放到application/core下面
...
class MY_Model extends CI_Model
{
    ...
    public function insert($data, $skip_validation = FALSE)
    {
        ...
        if ($data !== FALSE)
        {
            ...
            $ret = $this->_database->insert($this->_table, $data);
            /*
             *!!!这里的insert_id暂不知道如何设置参数,不用也可以
             * 当执行 INSERT 语句时，这个方法返回新插入行的ID。
             * If using the PDO driver with PostgreSQL, or using the Interbase driver,
             * this function requires a $name parameter, which specifies the appropriate sequence to check for the insert id.
             * */
...
//            $this->trigger('after_create', $insert_id);
    ...
    /**
     * Fetch a single record based on an arbitrary WHERE call. Can be
     * any valid value to $this->_database->or_where().
     */
    public function get_by_or_like_where($param)
    {
        $this->_database->like('name', $param);
        $this->_database->or_like('address', $param);
        return $this->get_all();
    }
}
...
class Node_Model extends MY_Model
{
    ...
    {
        parent::__construct();
        $this->_table = 'i2active_node';
        $this->primary_key = 'uuid';
        $this->_db = 'i2active';
    }


    public function search($param,$limit,$offset){
        return $this->as_array()->limit($limit,$offset)->get_by_or_like_where($param);
    }
    //as_array对象数组转为普通数组
    public function lists($limit,$offset){
        return $this->as_array()->limit($limit,$offset)->get_all();
    }
    //名称/ip升降序
    public function lists_order($limit,$offset,$criteria,$order){
        return $this->as_array()->limit($limit,$offset)->order_by($criteria,$order)->get_all();
    }
    public function getById($uuid){
        return $this->as_array()->get($uuid);
    }
    //查询全部
    ...
    //插入/更新数据
    public function add_or_update($data,$create_flag){
        if($create_flag){
    ...
    //判断某一个字段值是否重复,不重名返回false,重名返回true
    public function isFieldDuplicated($name,$value){
        $ret = $this->as_array()->get_many_by($name,$value);
        if(count($ret)==0){
            return false;
        }else{
            return true;
